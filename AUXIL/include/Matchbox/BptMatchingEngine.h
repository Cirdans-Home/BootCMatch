...
void BptMatchingEngine::rComputeMaxEdgWghtMatchingSglSrc(const BptGraph& graph,
  ...
  //if (mInlz == false) {
    ...
    *card = 0;
    ...
    rInlzRefinedForEdgWght(graph, sMateArr, tMateArr, sDualArr, tDualArr, &iniCard, &iniEdgWght, rvrs);
 /* } else {
    Size iniCard(0);
    Val iniEdgWght(0.0);
    rInlzGrdyForEdgWght
...
//timer.Start();
  ...
  *edgWght = 0.0;
  ...
  //std::cout << "search time " <<st<<" augmenting time "<< at << " dual update time "<<dut<<std::endl;
  if (maxCard == true) {
...
void BptMatchingEngine::rComputeMaxVtxWghtMatchingDrctGbl(
 ...
 std::vector<std::pair<Size, Val> > sExpsdLst2;
  ...
  //Size iniCard(0);
  //rInlzGrdyForVtxWght(graph,sMateArr,tMateArr,sExpsdLst2Arr, &iniCard, rvrs);
  //*card = iniCard;
  /*Size same=-1;
  Val wei=0.0;
  Size count=1;
  for (std::list<std::pair<Size, Val> >::const_iterator it2 = sExpsdLst.begin();
       it2 != sExpsdLst.end(); ++it2) {
       	Size sFirst = it2->first;
       	if(sVtxWghtArr[sFirst]==wei)
       	{
       		std::cout <<count <<" equal first vertex " <<same << " weight "<<sVtxWghtArr[same]<<" with vertex " << sFirst<<" weight "<<sVtxWghtArr[sFirst]<< std::endl;
		   count++;
		   }
		   same=sFirst;
		   wei=sVtxWghtArr[sFirst];
       }*/
  ...
  {
  	std::cout << " "<<it2->first<<":";
  Size testNumEdgs = sVtxVecArr[it2->first].size();
  const Size* testcurrentVtxArr = (testNumEdgs == 0) ? NULL : &sVtxVecArr[it2->first][0];
         
	    for (Size i = 0; i < testNumEdgs; ++i) {
	    	std::cout << " " <<testcurrentVtxArr[i];
	    }
	    std::cout<< std::endl;
}*/
  /*Size eCount=0;
  ...
  for (Size current = 0; current < sNumVtxs; ++current) {
    ...
    //if (augPathLenBnd == cInfSize) {
      rFindAugPathCardSglSrcBfs<ItmQue>
        (graph, sMateArr, tMateArr, sPtrArr, sSttArr, sPrcsbQue, sPrcsdQue,
         sFirst, &sLast, &tLast, rvrs/*,&eCount*/);
    /*} else {
      rFindAugPathCardSglSrcBfsBnd<ItmQue>
        (graph, sMateArr, tMateArr, sPtrArr, sLenArr, sSttArr, sPrcsbQue,
  ...
  //std::cout <<sNumVtxs<<" "<<feCount<<" "<<fvCount<<" "<<seCount<<" "<<svCount<<" ";
  *vtxWght = 0.0;
  for (Size s = 0; s < sNumVtxs; ++s) {
    if (sMateArr[s] != cNullItm) {
      *vtxWght += sVtxWghtArr[s];
     // *vtxWght += tVtxWghtArr[sMateArr[s]];
    }
  }
  /*std::cout.setf(std::ios::left | std::ios::scientific | std::ios::showpoint);
  std::cout.precision(mPrecision);
  std::cout << "weights " <<*vtxWght << std::endl;
	...
	*card = 0;
	...
	{	  
		...
		for(Size first = 0; first < tNumVtxs; ++first){
	        ...
	        {
	        	//std::cout << "no mate point to " <<vfirst << " its mate "<< curr <<" point to "<< mateArr[curr]<< std::endl;
	           	tMateArr[first]=cNullItm;
	           	done=false;
			}
			else
			{
				//*vtxWght=*vtxWght+ (tVtxWghtArr[first]+sVtxWghtArr[curr]);
				//*card=*card+1;
				//tMateArr[first]=curr;
				//sMateArr[curr]=first;
				tAlive[first]=0;
  ...
  for (Size first = 0; first < sNumVtxs; ++first)
   {
    ...
    {  	
	      ...
	      Size t = currentVtxArr[i];	   
				if((sVtxWghtArr[current] > tws[t]) /*|| (sVtxWghtArr[current] == tws[t] && tMateArr[t]>current)*/)
					...
					else if (tMateArr[t] !=cNullItm && sVtxWghtArr[current] == tws[t] && tMateArr[t]>current)
					{
						partner = t;
    				    heaviest = sVtxWghtArr[current];
    				    //std::cout << "equal vertex " <<current << " weight "<<sVtxWghtArr[current]<<" with vertex " << tMateArr[t]<<" weight "<<tws[t]<< std::endl;
						break;   
					}
	    }
	    done = true;
		...
		//std::cout << "1 vertex " <<current << std::endl;
	}
}
timer.Stop();
timer.Print();
  std::cout << "in suitor edges checked: " <<nedgesProfile << std::endl;
  std::cout << "annul: " <<annul << std::endl;
  *vtxWght = 0.0;
  Size mate =-1;
  for (Size t = 0; t < tNumVtxs; ++t) {
    if (tMateArr[t] != cNullItm) {
  ...
  std::cout << "weights " <<*vtxWght << std::endl;
  std::cout << "card " <<c << std::endl;
  std::cout.unsetf(std::ios::left | std::ios::scientific | std::ios::showpoint);*/
}
...
//timer.Start();
  ...
  // std::vector<Size> sExpsdLst;
  //ResizeVector<Size>(&sExpsdLst, sNumVtxs);
  //Size * sExpsdLstArr=  (sNumVtxs == 0) ? NULL : &sExpsdLst[0];
...
//timer.Start();
  ...
  /*for (Size sFirst = 0; sFirst < sNumVtxs; ++sFirst) {
    sExpsdLstArr[sFirst]=sFirst; 
}
   std::sort(sExpsdLst.begin(),sExpsdLst.end(), compare_indirect_index <Val> ( sVtxWghtArr ));*/
//const std::pair<Size,Val> * ptr = (sNumVtxs > 0) ? sExpsdLst2.data() : NULL;

////const Size* lastInt = &sExpsdLst[sNumVtxs - 1];
//Size * pInt = &sExpsdLst[0];
Size sFirst=0;
//Size nedgesProfile=0;
    for (Size current = 0; current < sNumVtxs; ++current) {
    //for( ; pInt <= lastInt; ++pInt){
    	sFirst =  sExpsdLst2Arr[current].first;
    	//Size sFirst =  ptr[current].first;
    	//sExpsdLst2Arr++;
    	//Size sFirst =  (*pInt).first;
    	 //sFirst =  sExpsdLstArr[current];
    	//if(sMateArr[sFirst]!=cNullItm)
    	//continue;
...
//timer.Stop();
//timer.Print();
//std::cout << "in 1/2 edges checked: " <<nedgesProfile << std::endl; 
  *vtxWght = 0.0;
  for (Size s = 0; s < sNumVtxs; ++s) {
    if (sMateArr[s] != cNullItm) {
    	++(*card);
...
void BptMatchingEngine::rComputeTwoThirdVtxWghtMatchingDrctGbl(
  ...
  *card = 0;
  ...
  //std::cout << "in 2/3 edges checked: " <<nedgesProfile << std::endl;  
  *vtxWght = 0.0;
  for (Size s = 0; s < sNumVtxs; ++s) {
    if (sMateArr[s] != cNullItm) {
      *vtxWght += sVtxWghtArr[s];
...
void BptMatchingEngine::rComputeHalfEdgWghtMatching(
  ...
  *edgWght=0;
  ...
  ResizeVector<Size>(&smarkVec, sNumVtxs);
  Size* smarkArr = (sNumVtxs == 0) ? NULL : &smarkVec[0];
  if (smarkArr != NULL) {
    std::fill(&smarkArr[0], &smarkArr[sNumVtxs], 0);
  }
  
  std::vector<Size> tmarkVec;
  ResizeVector<Size>(&tmarkVec, tNumVtxs);
  Size* tmarkArr = (tNumVtxs == 0) ? NULL : &tmarkVec[0];
  if (tmarkArr != NULL) {
